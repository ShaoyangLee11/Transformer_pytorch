...
def translate(sentence:str):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            prob=model.project(decoder_output[:,-1,:])                          # (1,seq_len,d_model)-->(1,1,vocab_size)

            _,next_word=torch.max(prob,dim=-1)

            decoder_input=torch.cat([decoder_input,torch.full(size=(1,1),fill_value=next_word.item())],dim=-1)
